...
class RedisBFSketch:
    ...
    def chunk(self, batch_of_strings):
        text_pipeline = build_text_pipeline_fn(width=self.width, stride=1, apply_code_processor=True)
        return utils.flatten_batched(text_pipeline(batches_of_text=batch_of_strings))


    def contains_from_text(self, documents, sort_chains_by_length=True):
